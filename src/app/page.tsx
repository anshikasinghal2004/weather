...
import convertWindSpeed  from "@/utils/convertWind";
import  getDayOrNightIcon  from "@/utils/getDayOrNight";
import  metersToKilometers  from "@/utils/convertMeter";
...
export default function Home() {
  ...
  const [loadingCity] = useAtom(loading);

  const { isLoading, error, data, refetch } = useQuery<WeatherData>(
    "repoData",
    async () => {
      const { data } = await axios.get(
        `https://api.openweathermap.org/data/2.5/forecast?q=${place}&appid=7d9a5243c005c226475ac059e331f6d4`
      );
      return data;
    }
  );
                ...
                <Container className=" gap-10 px-6 items-center">
                  ...
                  <div className=" flex flex-col px-4 ">
                    <span className="text-5xl">
                      {convert(firstData?.main.temp ?? 296.37)}°
                    </span>
                    ...
                    <p className="text-xs space-x-2">
                      <span>
                        {convert(firstData?.main.temp_min ?? 0)}
                        °↓{" "}
                      </span>
                      <span>
                        {" "}
                        {convert(firstData?.main.temp_max ?? 0)}
                        °↑
                      </span>
                    </p>
                  </div>
